 ...
 * Detect current page.
 */
export const currentPage: string = (document.querySelector('main') as any).dataset.page;<% } %>
...
export class Resp {
   ...
   */
  public static get currWidth(): number {
   ...
   */
  public static get isDesk(): boolean {
 ...
 * Css class names.
 */
export const css: { [x: string]: string } = {
  active: 'active'
};
...
export const randomString = (length: number = 10): string =>
  Math.random().toString(36).substr(2, length > 10 ? length : 10);
 ...
 * @returns {Function}
 */
export const debounce = (func: () => any, context: any, wait: number, immediate: boolean) => {
  let timeout;
 ...
 * @returns {Function}
 */
export const throttle = (fn: () => any, threshold: number = 250, scope: any) => {
  let last, deferTimer;
